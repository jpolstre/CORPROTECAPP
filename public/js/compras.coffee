...
class ShoppingCart2
	...
	numItems:0
	itemsInCart:[]
	objAttribs:AuxObjAttribs2#for order.
	# upOrCreIdexesRows:[]
	...
	editRow:(trJq)->
		...
		modalEditar = new Modal
			...
			tipo:'formulario'
			contenido: formHtmlEditarItem2
			despuesDeMostrar:(modalJq)->
				for item, i in AuxObjAttribs2 when item isnt 'subtotal'
...
$(document).ready ->
	...
	io.on 'compras:shopp', (data)->
		...
		# 		tablaCompras.add(ShoppingCart2.itemsInCart[i])
		# 	else#update.
	...
	tablaComprasJq = ctnComprasJq.find('#productos-registrados')
	...
	showHideMenujq = ctnComprasJq.find('div.show-hide-colms')
	$(document).ajaxComplete (event, xhr, settings)->
		tablaComprasJq.css('width', '')
		showHideMenujq.show()
	
	#hide/show columns.
	ctnComprasJq.find('ul.dropdown-cols input:checkbox').on 'click', (event)->
	...
	#tabs
	btnsTabs = ctnComprasJq.find('button.btns-tabs')
	ctnsTabs = ctnComprasJq.find('div.tab-pane')
	...
	#carousel.
	actualNum = 0
	carouselItem = ctnComprasJq.find('div#item-compra').carousel interval: false
	...
	titlePanelJq = ctnComprasJq.find('span#title-panel')
	itemElegido = {}
	...
	formCompraJq = ctnComprasJq.find('form#nueva-compra')
	...
	#btn clear.
	btnClearJq = ctnComprasJq.find('button#btn-clear-form').on 'click', (evt)->
		imputsFormCompraJq.val ''
		validador.ocultarMensajes()
		itemElegido = {}
		valArray.codigo.focus()
	...
	ShoppingCart2.addTo ctnComprasJq.find('div.table-responsive:eq(1)')#ok
