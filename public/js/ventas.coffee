...
numItemsJq = ctnVentasJq.find('span#num-items')
msgCarritoJq = ctnVentasJq.find('div#msg-carrito')
...
class ShoppingCart
	...
	calcSubtotal:()->#y suma total de paso.
		# @tipoPrecio = tipo
		index = if @tipoPrecio is 'recibo' then 4 else 5 
		sum = 0
...
$(document).ready ->
	...
	tablaVentasJq = ctnVentasJq.find('#productos-registrados')
	...
	showHideMenujq = ctnVentasJq.find('div.show-hide-colms')
	$(document).ajaxComplete (event, xhr, settings)->
		tablaVentasJq.css('width', '')
		showHideMenujq.show()
	
	#hide/show columns.
	ctnVentasJq.find('ul.dropdown-cols input:checkbox').on 'click', (event)->
	...
	#carousel.
	actualNum = 0
	carouselItem = ctnVentasJq.find('div#item-venta').carousel interval: false
	ctnVentasJq.find('ol.breadcrumb').on 'click', 'a', (evt)->
		evt.preventDefault()
	...
	ctnVentasJq.find('input.tipo_precio').on 'change', ->
		shoppingCart.tipoPrecio = $(this).val()
		shoppingCart.calcSubtotal() if shoppingCart.numRows > 0
					
	shoppingCart = new ShoppingCart()

	shoppingCart.addTo ctnVentasJq.find('div.table-responsive:eq(1)')#ok
